...
export async function proxy(request:NextRequest){
  ...
  if(path.startsWith("/anmelden")||path.startsWith("/einladung")||path.startsWith("/api/auth")||path.startsWith("/api/invitations/accept")||path.startsWith("/_next")||path.startsWith("/icons")||path==="/logo-finanzplaner.jpeg"||path==="/manifest.webmanifest") return NextResponse.next();
