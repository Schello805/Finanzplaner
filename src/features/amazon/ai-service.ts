...
import { categorizeWithAi, resolveModelPrice } from "@/features/ai/provider";
...
export const AMAZON_AI_BATCH_SIZE = 25;
...
async function pending(userId: string, ids?: string[], excludedIds: string[] = []) {
  ...
  const base = [
    eq(amazonOrderItems.ownerMemberId, member.id),
    isNull(amazonOrderItems.categoryId),
    isNull(amazonOrderItems.aiAnalyzedAt),
    sql`${amazonOrderItems.quantity} > 0`,
    sql`${amazonOrderItems.orderTotal} > 0`,
    sql`${amazonOrderItems.status} !~* 'cancel|storniert'`,
  ];
  ...
  const databaseRows = await db.select({
    ...
    date: amazonOrderItems.orderDate,
    amount: amazonOrderItems.unitPrice,
    tax: amazonOrderItems.unitTax,
    quantity: amazonOrderItems.quantity,
    currency: amazonOrderItems.currency,
    productName: amazonOrderItems.productNameEncrypted,
    department: amazonOrderItems.department,
  }).from(amazonOrderItems).where(and(...filters)).limit(ids ? 100 : AMAZON_AI_BATCH_SIZE);
...
export async function getAmazonAiPreview(userId: string, excludedIds: string[] = []) {
  const state = await pending(userId, undefined, excludedIds);
  if (!state.rows.length) return { ...state, provider: undefined, model: undefined, price: null };
  const ai = await aiSettings();
  return { ...state, provider: ai.provider, model: ai.config.model, price: resolveModelPrice(ai.provider, ai.config.model, ai.config) };
}

export async function processAmazonAiBatch(userId: string, ids?: string[]) {
  const { member, rows } = await pending(userId, ids);
  if (!rows.length) return { applied: 0, suggestions: [], categoryProposals: [], analyzedIds: [], estimatedCostEur: 0, pricingAvailable: false };
  const ai = await aiSettings();
  ...
  const [preferences] = await db.select({ level: userPreferences.aiAutoAcceptLevel }).from(userPreferences).where(eq(userPreferences.userId, userId)).limit(1);
  ...
  let applied = 0;
  const analyzedIds = rows.map((row) => row.id);
  for (const item of result.data.results) {
    if (!analyzedIds.includes(item.id)) continue;
  ...
  const returnedIds = new Set(result.data.results.map((item) => item.id));
  const unansweredIds = analyzedIds.filter((id) => !returnedIds.has(id));
  if (unansweredIds.length) {
    await db.update(amazonOrderItems).set({
      aiSuggestionReason: "Die KI hat für diesen Artikel keine Antwort geliefert.",
      aiAnalyzedAt: new Date(),
      updatedAt: new Date(),
    }).where(and(inArray(amazonOrderItems.id, unansweredIds), eq(amazonOrderItems.ownerMemberId, member.id), isNull(amazonOrderItems.categoryId)));
  }
  ...
  await db.insert(aiUsage).values({ householdId: member.householdId, userId, provider: ai.provider, model: ai.config.model, purpose: "amazon-categorization", inputTokens: result.usage.inputTokens, outputTokens: result.usage.outputTokens, estimatedCostEur: estimatedCostEur?.toFixed(6) });
  await writeAudit("ai", "Amazon-Artikel wurden mit KI kategorisiert.", { userId, metadata: { count: rows.length, applied, suggestions: suggestions.length, provider: ai.provider } });
  return { applied, suggestions, categoryProposals, analyzedIds, usage: result.usage, estimatedCostEur, pricingAvailable: Boolean(price) };
}
