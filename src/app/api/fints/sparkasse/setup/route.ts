...
export async function POST(request:Request){try{
 ...
 if(body.action==="discover"){
  ...
  const config=FinTSConfig.forFirstTimeUse(body.productId,finTsProductVersion(),body.endpoint,body.blz,body.userId,body.pin);
 ...
 }catch(error){console.error("[FinTS setup] Anfrage abgelehnt",error instanceof Error?error.message:error);return NextResponse.json({error:error instanceof Error?error.message:"Sparkassen-Verbindung konnte nicht eingerichtet werden."},{status:400})}}
