...
export async function proxy(request:NextRequest){
  ...
  if(path.startsWith("/anmelden")||path.startsWith("/api/auth")||path.startsWith("/_next")||path.startsWith("/icons")||path==="/logo-finanzplaner.jpeg"||path==="/manifest.webmanifest") return NextResponse.next();
