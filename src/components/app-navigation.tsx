  ...
  return <>
    <aside className="fixed inset-y-0 left-0 z-20 hidden w-[250px] border-r border-[var(--border)] bg-[var(--surface)] px-4 py-6 md:block">
      ...
      <nav aria-label="Hauptnavigation" className="space-y-1.5">
        {links.filter(link=>link.href!=="/admin"||summary?.isAdmin).map(({ href, label, icon: Icon }) => {
    ...
    <nav aria-label="Mobile Hauptnavigation" className={`fixed bottom-0 left-0 right-0 z-30 grid ${summary?.isAdmin?"grid-cols-5":"grid-cols-4"} border-t border-[var(--border)] bg-[var(--surface)] px-1 pb-[env(safe-area-inset-bottom)] md:hidden`}>
      {links.filter(link=>link.href!=="/admin"||summary?.isAdmin).map(({ href, label, icon: Icon }) => { const active = href === "/" ? pathname === "/" : pathname.startsWith(href); return (
