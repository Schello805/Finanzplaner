...
export function sanitizeLogMetadata(input: Record<string, unknown>) {
  return Object.fromEntries(Object.entries(input).filter(([key]) => !/(password|secret|token|purpose|iban|account|prompt|response)/i.test(key)).map(([key, value]) => [key, typeof value === "string" ? redactSensitiveText(value).slice(0, 500) : value]));
}
