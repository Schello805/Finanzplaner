...
export async function POST(request:NextRequest){const token=request.cookies.get(SESSION_COOKIE)?.value;if(!token)return NextResponse.json({error:"Nicht angemeldet."},{status:401});
  ...
  await writeAudit("password-changed","Das persönliche Passwort wurde geändert.",{userId:user.id});
  const newToken=await createSessionToken({...session,mustChangePassword:false});const response=NextResponse.json({ok:true,pwnedCheck:pwned.status});response.cookies.set(SESSION_COOKIE,newToken,sessionCookieOptions);return response;}
